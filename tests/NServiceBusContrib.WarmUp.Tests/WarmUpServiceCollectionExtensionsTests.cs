...
{
    [Fact]
    public void AddNServiceBusWarmUp_registers_the_readiness_registry()
    {
        var provider = new ServiceCollection()
            .AddNServiceBusWarmUp()
            .BuildServiceProvider();

        Assert.IsType<EndpointReadinessRegistry>(provider.GetRequiredService<IEndpointReadinessRegistry>());
    }
