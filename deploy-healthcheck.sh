#!/usr/bin/env bash
#
# Deploy NServiceBusContrib.HealthCheck to NuGet.
#
# Usage:
#   NUGET_API_KEY=<key> ./deploy-healthcheck.sh (-major | -minor | -patch) [options]
#
...
#
# NOTE: this package depends on NServiceBusContrib.WarmUp at WarmUp's current
# <Version>. Publish WarmUp (deploy-warmup.sh) at that version FIRST, otherwise
# this package will reference a WarmUp version that isn't on the feed.
#
# Options:
...
PROJECT="src/NServiceBusContrib.HealthCheck/NServiceBusContrib.HealthCheck.csproj"
PACKAGE_ID="NServiceBusContrib.HealthCheck"
WARMUP_PROJECT="src/NServiceBusContrib.WarmUp/NServiceBusContrib.WarmUp.csproj"
...
# --- warn about the WarmUp dependency version this package will reference ---
warmup_version="$(grep -oP '(?<=<Version>)[^<]+' "$WARMUP_PROJECT" | head -1 || true)"
echo ">> will depend on NServiceBusContrib.WarmUp ${warmup_version:-<unset>} — ensure it is published"
...
#     into the WarmUp project reference) ---
echo ">> running tests"
